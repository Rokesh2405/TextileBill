...
//    echo $_REQUEST['year'];
    if ($_REQUEST['page'] != '') {
        ...
        echo $page;
        //exit;
        $dateformate = $db->prepare("SELECT * FROM `loan` WHERE `status`='1' AND `returnstatus`='1' OR `returnstatus`='2' LIMIT $page ");
        $dateformate->execute();
        echo "SELECT * FROM `loan` LIMIT $page";
    } else {
        $dateformate = $db->prepare("SELECT * FROM `loan` WHERE `status`='1' AND `returnstatus`='1' OR `returnstatus`='2' ");
        $dateformate->execute();
    }
...
//                            echo $orderdate['0'];
                        ...
                        <tbody>
                            <tr style="font-size:19px">
                                <td><?php echo $i; ?></td>
                                <td><?php echo  $orderdate['customerid']; ?></td>
                                <td><?php echo $orderdate['name']; ?></td>
                                <td><?php echo  $orderdate['receipt_no']; ?></td>
                                ...
                                <td><?php echo number_format($orderdate['interest'], 2, '.', ''); ?></td>
                                <td><?php echo number_format($orderdate['amount'], 2, '.', ''); ?></td>
                                <td><?php
                                    if ($orderdate['status'] == '1' && $orderdate['returnstatus'] == '1' || $orderdate['returnstatus'] == '2') {
                                        echo 'Pawned';
                                    } else {
                                        echo 'Returned';
                                    }
...
<!--                                <td><?php //echo $orderdate['currentdate']; ?></td>
                                ...
                                <?php //echo $orderdate['amount']; ?></td>-->
                                <td><?php $t1 = number_format($orderdate['interest'], 2, '.', ''); 
                                    $t2 = number_format($orderdate['amount'], 2, '.', ''); 
                                    $t3 = $t2 - $t1;
                                    echo $t3;
                                ...
                                <td><?php echo number_format($orderdate['netweight'], 2, '.', ''); ?></td>
                                <td><?php
                                   if ($orderdate['status'] == '1' && $orderdate['returnstatus'] == '1' || $orderdate['returnstatus'] == '2') {
                                        echo 'Pawned';
                                    } else {
                ...
                echo '<label>No row displayed yet1';
            }
            ?>
            <tfoot>
                <tr>
    ...
    $yearqry = $db->prepare("SELECT * FROM `loan` WHERE `date`=? ");
        ...
        <!--                                    <th data-sortable="false" align="center" style="text-align: center; padding-right:0; padding-left: 0; width: 10%;">Edit</th>
                ...
                while ($year = $yearqry->fetch(PDO::FETCH_ASSOC)) {
                    ?><tbody>
                        ...
                        <tr style="font-size:19px">
                            <td><?php echo $i; ?></td>
                            <td><?php echo $year['customerid']; ?></td>
                            <td><?php echo $year['name']; ?></td>
                            <td><?php echo $year['receipt_no']; ?></td>
                            <td><?php echo $year['netweight']; ?></td>
                            <td><?php echo $year['date']; ?></td>
<!--                            <td><?php //echo $year['currentdate']; ?></td>
                            <td><?php //echo $year['amount']; ?></td>-->
                            <td><?php echo number_format($year['interest'], 2, '.', ''); ?></td>
                            <td><?php echo number_format($year['amount'], 2, '.', ''); ?></td>
                            <td><?php
                                if ($orderdate['status'] == '1' && $orderdate['returnstatus'] == '1' || $orderdate['returnstatus'] == '2') {
                                        echo 'Returned';
                                    } else {
        ...
        <!--                                    <th style="margin:0px auto;"><button type="submit" class="btn btn-danger" name="delete" id="delete" value="Delete" onclick="return checkdelete('chk[]');"> DELETE </button></th>-->
            ...
            echo '<label>No row displayed yet2</label>';
        }
        ?>
    </div>
    <?php
