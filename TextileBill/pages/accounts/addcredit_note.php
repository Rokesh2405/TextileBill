...
if ($_REQUEST['id'] != '') {
    ...
    if ($fetched['id'] == '') {
        header("location:" . $sitename . 'accounts/credit_note.htm');
        exit;
    }
}
    ...
    if ($_REQUEST['id'] == '') {
        ...
        foreach($paynow as $keys => $pays){
            ...
            if($pays != ''){
                ...
                $insert->execute(array('C',$supplier, date("Y-m-d", strtotime($date)), $get['ref_id'], $get['bill_no'], $get['bill_date'], $get['amount'], $pays, ($get['amount'] - $pays), $pays, $paytype, $bank, $cheque, date("Y-m-d",strtotime($cdate)), $desc, $ip, $_POST['bill_id'][$keys]));
                ...
                add_transaction($get['bill_no'], date("Y-m-d"), $get['ledger_id'], $pays, $lsid, 'Credit_Note', 'C');
                add_transaction($get['bill_no'], date("Y-m-d"), $ledg, $pays, $lsid, 'Debit_Note', 'D');
        ...
        if ($paynow != '') {
            ...
            transaction_del($fetched['id'], 'Credit_Note');

            $uts = $db->prepare("UPDATE `transactions` SET `return_amount` = ((`return_amount` - ?) + ?) WHERE `id`=?");
            $uts->execute(array($fetched['pay_now_amount'], $paynow, $_POST['bill_id']));

            add_transaction($get['bill_no'], date("Y-m-d"), $get['ledger_id'], $paynow, $fetched['id'], 'Credit_Note', 'C');
            add_transaction($get['bill_no'], date("Y-m-d"), $ledg, $paynow, $fetched['id'], 'Debit_Note', 'D');
        }
        $get1 = $db->prepare("SELECT * FROM `accounts_payment` WHERE `id`=? AND `type`=?");
        $get1->execute(array($_REQUEST['id'],'C'));
        $fetched = $get1->fetch(PDO::FETCH_ASSOC);
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="<?php echo $sitename; ?>accounts/credit_note.htm"><i class="fa fa-circle-o"></i>Credit Note </a></li>
            <li class="active"><?php
                if ($_REQUEST['lid'] != '') {
                    echo 'Edit';
                } else {
                ...
                <div class="box-body">
                    ...
                    <div class="panel panel-info">
                        <div class="panel-heading">
                            <div class="panel-title">Credit Note</div>
                        </div>
...
<!--                                <div class="col-md-4">
                                    ...
                                    if ($_REQUEST['id'] != '') {
                                        $disabled = 'disabled';
                                        echo "<input type='hidden' name='supplier' />";
                                    }
                                    ?>
                                    <select onchange="getsuptrans(this.value)" name="supplier" id="supplier" class="form-control" <?php echo $disabled; ?> required="required">
                                        <option value="">Please select</option>
                                        <?php
                                        $sup = $db->prepare("SELECT * FROM `suppliers` WHERE `Status`=? ");
                                        $sup->execute(array(1));
                                        while ($s = $sup->fetch()) {
                                            $selc = '';
                                            if ($fetched['supplier'] == $s['SupID']) {
                                                $selc = 'selected';
                                            }
                                            echo '<option value="' . $s['SupID'] . '" ' . $selc . '>' . $s['Supplier_name'] . '</option> ';
                                        }
                                        ?>                                               
                                    </select>
                                </div>-->
                                <div class="col-md-4">
                                    <label><br /></label>
                                    <label><br /><input type="checkbox" name="no_balance" id="no_balance" onchange="getsuptrans($('#supplier').val());" value="1" /> No pending balance also</label>
                                </div>
                                <div class="col-md-4">
                                        ...
                                        <div class="table-responsive">
                                         ...
                                         $fi = '('.Currency($sy['code']).')'; $fi_hi = Currency($sy['code']);  } ?>
                                            <table class="table" width="100%">
                                                ...
                                                <th style="width:22%;text-align:center">Total Amount <?php echo $fi; ?> <label class="curr"></label></th>
                                                <th style="width:20%;text-align:center">Received Amount <?php echo $fi; ?> <label class="curr"></label></th>
                                                <th style="width:18%;text-align:center">Balance <?php echo $fi; ?> <label class="curr"></label></th>
                                                ...
                                                <tbody id="data_pay">
                                                    ...
                                                    <tr>
                                                        <td style="width:10%"><?php echo $fetched['bill_no']; ?></td>
                                                        <td style="width:15%"><?php echo date("d-m-Y", strtotime($fetched['bill_date'])); ?></td>
                                                        <td style="width:22%;text-align:center"><?php echo number_format($fetched['amount'],2); ?></td>
                                                        <td style="width:20%;text-align:center"><?php echo number_format($fetched['received_amount'],2); ?></td>
                                                        <td style="width:18%;text-align:center"><?php echo number_format($fetched['balance_amount'],2); ?></td>
                                                        <td style="width:20%;text-align:center"><input type="text" class="form-contorl" onkeyup="paynow1212()" id="paynow<?php echo $i; ?>" name="paynow" value="<?php echo $fetched['received_amount'] ?>" /></td>
                                                    </tr>
                                                    <?php
                                                    $i++;
                                                }
...
<!--                                                <h4 id="bal_amt">Total Amount <?php echo $fi; ?> : <?php echo number_format(($fetched['pay_now_amount']) ? $fetched['pay_now_amount'] : '0.00',2); ?></h4>
                ...
                <div class="box-footer">
                            <div class="row">
                                <div class="col-md-6">
                                    <a href="<?php echo $sitename; ?>accounts/credit_note.htm">Back to Listings page</a>
                                </div>
                                <div class="col-md-6">
