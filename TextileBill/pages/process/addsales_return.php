...
$als = FETCH_all("SELECT * FROM `sales_return` WHERE `bill_number`=? AND date(`cudate`)=?", $salesorder_billno,date('Y-m-d',strtotime($salesorder_date)));
if($als['id']!='') {
$link22 = FETCH_all("SELECT * FROM `sales_return` WHERE `id`=?", $als['id']);   
}
else
{
$link22 = FETCH_all("SELECT * FROM `online_order` WHERE `bill_number`=? AND date(`cudate`)=?", $salesorder_billno,date('Y-m-d',strtotime($salesorder_date)));    
}
}
                ...
                <div class="box-body">
                    ...
                    <form name="searchform" method="post">
                     <div class="row">
                        ...
                        <input type="date" name="salesorder_date" id="salesorder_date" class="form-control" required value="<?php echo $_REQUEST['salesorder_date']; ?>">
                        </div>
                        <div class="col-md-3">
                        <label>Sales Order Bill No</label>
                        <input type="text" name="salesorder_billno" id="salesorder_billno" class="form-control" required value="<?php echo $_REQUEST['salesorder_billno']; ?>">
                        </div>
                         <div class="col-md-4">
                            <br>
...
if($getrecr['id']==''){
                                                    ...
                                                    $qty=$objectsss1list['qty'];
                                                     ...
                                                     $resa = $db->prepare("INSERT INTO `temp_sales_details` (`style`,`meter`,`orgrate`,`hsn`,`gstresult`,`object_id`,`bill_no`,`product_name`,`qty`,`rate`,`total`,`mrp_price`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ");
            $resa->execute(array($style,$meter,$orgrate,$hsn,$gstresult,$object_id,$bill_no,$product_name,$qty,$rate,$total,$mrp_price));
}
...
<input type="hidden" style="text-align: right;" name="orgrate[]" readonly="readonly"  id="orgrate[]" class="form-control orgrate"/>
    ...
    <input type="text" style="text-align: right;" name="rate[]" id="rate[]" readonly="readonly" class="form-control rate" value="" /><input type="hidden" name="ratehidden" id="ratehidden" value="" onkeyup="quantitycalculation(this.value)"/></td>
...
<td>
<input type="text" style="text-align: right;" name="mrp_price[]"  readonly="readonly" id="mrp_price" class="form-control mrp_price" />
 <input type="hidden" style="text-align: right;" name="orgrate[]" readonly="readonly"  id="orgrate[]" class="form-control orgrate" />
...
<table border="1" cellpadding="10" style="width:372px;font-size:13px; font-weight:bold;" align="right">
  ...
  <td colspan="2">&nbsp; </td>
                ...
                <div class="box-footer">
                    <div class="row">
                        <div class="col-md-4">
                            <a href="<?php echo $sitename; ?>pages/process/sales_return.php" target="_blank" style="font-size:19px;">Back to Listings page</a>
                        </div>
                        <div class="col-md-4">
                        <?php if($_REQUEST['lid']!='') {
