...
$chkcredit = FETCH_all("SELECT * FROM `credit_payment` WHERE `id`=? AND `type`=?", $purchase_id,'purchase');
if($chkcredit['balance_amt']>=$current_amt) {  
  $advamt=$chkcredit['advance_amt'] + $current_amt;
  $balance_amt=$chkcredit['purchase_amt'] - $advamt;
$resa = $db->prepare("UPDATE `credit_payment` SET `inputamt`=?,`advance_amt`=?,`balance_amt`=? WHERE `id`=? AND `type`=?");
$resa->execute(array($current_amt,$advamt,$balance_amt,$purchase_id,'purchase'));


//Credit History
$cresa = $db->prepare("INSERT INTO `credit_history` (`type`,`billno`,`supplier_id`,`purchase_id`,`purchase_amt`,`advance_amt`,`balance_amt`,`inputamt`) VALUES (?,?,?,?,?,?,?,?)");
$cresa->execute(array('purchase',$chkcredit['billno'],$chkcredit['supplier_id'],$chkcredit['purchase_id'],$chkcredit['purchase_amt'],$chkcredit['advance_amt'],$chkcredit['balance_amt'],$current_amt));
//Credit History
...
$message1 = $db->prepare("SELECT * FROM `credit_payment` WHERE `id`!='0' AND `type`='purchase' ");	
$m=1;
$message1->execute();
...
<td><?php echo getpurchase('bill_number',$message['purchase_id']); ?></td>
